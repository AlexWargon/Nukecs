...
namespace Wargon.Nukecs.Tests
{
    ...
    public static unsafe class RuntimeQueryIter4PagedApi
    {
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static RuntimePagedIter<RuntimePagedRefs<T1, T2, T3, T4>> iter_paged_runtime<T1, T2, T3, T4>(
            ...
            where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent
        {
            ...
            return new RuntimePagedIter<RuntimePagedRefs<T1, T2, T3, T4>>(raw.Ptr);
        }

        // Four data components plus a filter (including a bare tag). Tags have no tuple slot.
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static RuntimePagedIter<RuntimePagedRefs<T1, T2, T3, T4>> iter_paged_runtime<T1, T2, T3, T4, TFilter>(
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static RuntimePagedPoolIter<RuntimePagedRefs<T1, T2, T3, T4>> iter_paged_pool_runtime<T1, T2, T3, T4>(
            this in Query<T1, T2, T3, T4> query)
            where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent
            where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent
        {
            query.TryGetQuery(out var raw);
            return new RuntimePagedPoolIter<RuntimePagedRefs<T1, T2, T3, T4>>(raw.Ptr);
        }

        // Four data components plus a filter (including a bare tag). Tags have no tuple slot.
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static RuntimePagedPoolIter<RuntimePagedRefs<T1, T2, T3, T4>> iter_paged_pool_runtime<T1, T2, T3, T4, TFilter>(
            this in Query<T1, T2, T3, T4, TFilter> query)
            where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent
            where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent where TFilter : unmanaged
        {
    ...
    public unsafe struct RuntimePagedColumn
    {
        private byte* data;
        ...
        public byte* At(int row, int pageIndex, int slot)
        {
            if (pool == null) return data + row * size;
            // Read the current page table through its stable owner: no cached table
            // pointer that could become stale if another page is added.
            if ((uint)pageIndex < (uint)pool->Chunks.capacity)
            {
                ref var page = ref pool->Chunks.Ptr[pageIndex];
                if (page.isCreated == 1) return page.buffer.Ptr + slot * size;
            }
            return MissingPage(pageIndex, slot);
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        private byte* MissingPage(int pageIndex, int slot)
        {
            // Legacy Add<Pool>()/ECB paths can publish membership before allocating
            // a page. Preserve GetPtr's lazy initialization only for that case.
            return pool->GetPtr(pageIndex * Chunk.MAX_CHUNK_SIZE + slot);
        }
    }

    public unsafe struct RuntimePagedRows
    {
        public RuntimePagedColumn A, B, C, D;
    ...
    public unsafe interface IRuntimePagedTuple : IRuntimeDenseTuple
    {
        void Init(ref RuntimePagedRows rows, World.WorldUnsafe* world);
        void Gather(ref RuntimePagedRows rows, int row);
        void SetArchetype(ref ArchetypeUnsafe archetype);
    }

    [StructLayout(LayoutKind.Sequential)]
    public unsafe struct RuntimePagedRefs<T1, T2, T3, T4> : IRuntimePagedTuple
        where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent
        where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent
    {
        private byte* a;
        ...
        public void Init(ref RuntimePagedRows rows, World.WorldUnsafe* world)
        {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void Gather(ref RuntimePagedRows rows, int row)
        {
            var entityId = rows.HasPools ? rows.PackedEntities[row] : 0;
            var page = entityId >> Chunk.CHUNK_INDEX_BITSFIFT;
            var slot = entityId & (Chunk.MAX_CHUNK_SIZE - 1);
            a = rows.A.At(row, page, slot);
            b = rows.B.At(row, page, slot) - a;
            c = rows.C.At(row, page, slot) - a;
            d = rows.D.At(row, page, slot) - a;
        }
    ...
    // Explicit pool path: the category check happens once, before enumeration.
    // Keeping a distinct iterator avoids adding a per-row mode branch to inline iteration.
    public unsafe ref struct RuntimePagedPoolIter<TTuple> where TTuple : unmanaged, IRuntimePagedTuple
    {
        private TTuple current;
        private RuntimePagedRows columns;
        ...
        public RuntimePagedPoolIter(QueryUnsafe* query)
        {
            current = default;
            columns = default;
            world = query->world;
            ...
            if (!columns.HasPools)
                throw new InvalidOperationException("The pool iterator requires at least one pool data component.");
            matches = query->matchingArchetypes.Ptr;
            matchCount = query->matchingArchetypes.Length;
            block = -1;
            rows = null;
            rowIndex = rowCount = 0;
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public RuntimePagedPoolIter<TTuple> GetEnumerator() => this;
    ...
    public unsafe ref struct RuntimePagedIter<TTuple> where TTuple : unmanaged, IRuntimePagedTuple
    {
        private TTuple current;
        private readonly World.WorldUnsafe* world;
        private readonly int* matches;
        ...
        public RuntimePagedIter(QueryUnsafe* query)
        {
            current = default;
            world = query->world;
            columns = default;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public RuntimePagedIter<TTuple> GetEnumerator() => this;
        public readonly TTuple Current { [MethodImpl(MethodImplOptions.AggressiveInlining)] get => current; }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool MoveNext() => current.Advance(end) || MoveNextGathered();

        [MethodImpl(MethodImplOptions.NoInlining)]
        private bool MoveNextGathered()
        {
