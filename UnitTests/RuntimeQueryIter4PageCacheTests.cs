...
{
    [TestFixture]
    public unsafe class RuntimeQueryIter4PageCacheTests
    {
        [Test]
        public void AllPoolSpecialization_Empty_LazyPages_AndAppendSnapshot()
        {
            ...
            try
            {
                ...
                var iterator=query.iter_cached_allpool_runtime();
                Assert.IsFalse(iterator.MoveNext()); Assert.IsFalse(iterator.MoveNext());
                var e=world.Entity();
                e.Add<RuntimeQi4PoolA>(); e.Add<RuntimeQi4PoolB>(); e.Add<RuntimeQi4PoolC>(); e.Add<RuntimeQi4PoolD>();
                world.Update(); query.Update(ref world,IntPtr.Zero);
                iterator=query.iter_cached_allpool_runtime();
                Assert.IsTrue(iterator.MoveNext());
                var (a,b,c,d)=iterator.Current;
                Assert.AreEqual(0,a.Read.Id); Assert.AreEqual(0,b.Read.Value);
                Assert.AreEqual(0,c.Read.Value); Assert.AreEqual(0,d.Read.Value);
                a.Get.Id=7; b.Get.Value=123; c.Get.Value=12.5; d.Get.Value=9;
                var arch=world.GetArchetype(typeof(RuntimeQi4PoolA),typeof(RuntimeQi4PoolB),typeof(RuntimeQi4PoolC),typeof(RuntimeQi4PoolD));
                arch.CreateEntity();
                Assert.IsFalse(iterator.MoveNext(),"Appended row must not extend the active block.");
                Assert.AreEqual(7,e.Get<RuntimeQi4PoolA>().Id); Assert.AreEqual(123,e.Get<RuntimeQi4PoolB>().Value);
                Assert.AreEqual(12.5,e.Get<RuntimeQi4PoolC>().Value); Assert.AreEqual(9,e.Get<RuntimeQi4PoolD>().Value);
            }
            finally { world.Dispose(); }
        }
        [Test]
        public void OnePoolSpecialization_SparseRows_AndUnequalSizes()
        {
            var world=World.Create(WorldConfig.Default16384);
            try
            {
                var entities=Populate(ref world);
                var query=new Query<Qi4A,Qi4B,Qi4C,RuntimeQi4PoolD>();
                Init(ref query,ref world);
                var expected=query.iter_paged_pool_runtime();
                var actual=query.iter_cached_aaap_runtime();
                var saved=actual.Current;
                var savedId=-1;
                while (expected.MoveNext())
                {
                    ...
                    var id=a.Read.Value;
                    Assert.AreEqual(entities[id].Get<Qi4B>(),b.Read);
                    Assert.AreEqual(entities[id].Get<RuntimeQi4PoolD>(),d.Read);
                    a.Get.Value+=10000; Assert.AreEqual(x.Read.Value,a.Read.Value); a.Get.Value-=10000;
                    if (savedId<0) { saved=actual.Current; savedId=id; }
                    var (snapshot,_,_,_)=saved; Assert.AreEqual(savedId,snapshot.Read.Value);
                }
                Assert.GreaterOrEqual(savedId,0);
                Assert.IsFalse(actual.MoveNext()); Assert.IsFalse(actual.MoveNext());
            }
            finally { world.Dispose(); }
        }
        [Test]
        public void TwoPoolsSpecialization_SparseRows_AndUnequalSizes()
        {
            var world=World.Create(WorldConfig.Default16384);
            try
            {
                var entities=Populate(ref world);
                var query=new Query<Qi4A,RuntimeQi4PoolB,Qi4C,RuntimeQi4PoolD>();
                Init(ref query,ref world);
                var expected=query.iter_paged_pool_runtime();
                var actual=query.iter_cached_apap_runtime();
                var saved=actual.Current;
                var savedId=-1;
                while (expected.MoveNext())
                {
                    ...
                    Assert.AreEqual((IntPtr)z.data,(IntPtr)c.data); Assert.AreEqual((IntPtr)w.data,(IntPtr)d.data);
                    var id=a.Read.Value;
                    Assert.AreEqual(entities[id].Get<RuntimeQi4PoolB>(),b.Read);
                    Assert.AreEqual(entities[id].Get<RuntimeQi4PoolD>(),d.Read);
                    a.Get.Value+=10000; Assert.AreEqual(x.Read.Value,a.Read.Value); a.Get.Value-=10000;
                    if (savedId<0) { saved=actual.Current; savedId=id; }
                    var (snapshot,_,_,_)=saved; Assert.AreEqual(savedId,snapshot.Read.Value);
        ...
        [Test]
        public void AllPoolSpecialization_PageGrowth_CurrentSnapshot_AndValidation()
        {
            var world = World.Create(WorldConfig.Default1024);
            try
            {
                ...
                for (var i=0;i<257;i++)
                {
                    ...
                    if (i%3==0) e.Add<Qi4Tag>();
                }
                world.Update();
                var query = new Query<RuntimeQi4PoolA, RuntimeQi4PoolB, RuntimeQi4PoolC, RuntimeQi4PoolD, None<Qi4Tag>>();
                Init(ref query, ref world);
                var expected=query.iter_paged_pool_runtime();
                var actual=query.iter_cached_allpool_runtime();
                Assert.IsTrue(actual.MoveNext()); Assert.IsTrue(expected.MoveNext());
                var saved=actual.Current;
                var (first,_,_,_)=saved;
                var savedId=first.Read.Id;
                // Force page-table relocation while the iterator has cached a live page.
                ...
                do
                {
                    ...
                    Assert.AreEqual(1000+a.Read.Id,b.Read.Value);
                    Assert.AreEqual(10.5+a.Read.Id,c.Read.Value);
                    Assert.AreEqual((byte)a.Read.Id,d.Read.Value);
                    a.Get.Id+=10000; Assert.AreEqual(x.Read.Id,a.Read.Id); a.Get.Id-=10000;
                    seen++;
                    var next=expected.MoveNext();
                    Assert.AreEqual(next,actual.MoveNext());
                    if (!next) break;
                } while (true);
                Assert.AreEqual(171,seen);
                Assert.IsFalse(actual.MoveNext());
                var (snapshot,_,_,_)=saved;
                Assert.AreEqual(savedId,snapshot.Read.Id);
                var mixed=new Query<Qi4A,Qi4B,Qi4C,RuntimeQi4PoolD>();
                Init(ref mixed,ref world);
                Assert.Throws<InvalidOperationException>(() => { mixed.iter_cached_allpool_runtime(); });
            }
            finally { world.Dispose(); }
        }
        [Test]
        public void DefaultPools_LazyPages_AndPageTableGrowth_RemainSupported()
        {
            ...
            try
            {
                ...
                AssertSameRows(query.iter_cached_runtime(), query.iter_direct_pool_runtime());
            }
            finally { world.Dispose(); }
        }
        ...
        private static void Check<T1, T2, T3, T4>(
            RuntimePageIter<RuntimeCachedRefs<T1, T2, T3, T4>> iterator, Entity[] entities,
            ...
            where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent
        {
            ...
            var saved = default(RuntimeCachedRefs<T1, T2, T3, T4>);
            var savedId = -1;
            ...
            where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent
        {
            ...
            Init(ref query, ref world);
            Check(query.iter_cached_runtime(), entities);
            if (ComponentType<T1>.Data.category == ComponentCategory.Pool ||
                ...
                Assert.Throws<InvalidOperationException>(() => { query.iter_direct_pool_runtime(); });
        }

        private static void AssertSameRows<T1, T2, T3, T4>(
            RuntimePageIter<RuntimeCachedRefs<T1, T2, T3, T4>> expected,
            RuntimePageIter<RuntimeDirectRefs<T1, T2, T3, T4>> actual)
            where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent
            where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent
        {
            while (expected.MoveNext())
        ...
        public void TagsOnlyFilter_NoTagPayload_InlineAndPoolQueries()
        {
            ...
            try
            {
                ...
                Check(with.iter_cached_runtime(), entities, e => e.Has<Qi4Tag>());
                var none = new Query<RuntimeQi4PoolA, Qi4B, Qi4C, Qi4D, None<Qi4Tag>>();
                Init(ref none, ref world);
                Check(none.iter_cached_runtime(), entities, e => !e.Has<Qi4Tag>());
                AssertSameRows(none.iter_cached_runtime(), none.iter_direct_pool_runtime());
                var tag = new Query<Qi4A, RuntimeQi4PoolB, Qi4C, Qi4D, Qi4Tag>();
                Init(ref tag, ref world);
                Check(tag.iter_cached_runtime(), entities, e => e.Has<Qi4Tag>());
                AssertSameRows(tag.iter_cached_runtime(), tag.iter_direct_pool_runtime());
                var nonePool = new Query<Qi4A, Qi4B, Qi4C, Qi4D, None<RuntimeQi4PoolA>>();
                Init(ref nonePool, ref world);
                Check(nonePool.iter_cached_runtime(), entities, e => !e.Has<RuntimeQi4PoolA>());

                var invalidTag = new Query<Qi4A, Qi4B, Qi4C, Qi4Tag>();
                Init(ref invalidTag, ref world);
                Assert.Throws<InvalidOperationException>(() => { invalidTag.iter_cached_runtime(); });
                var invalidFifth = new Query<Qi4A, Qi4B, Qi4C, Qi4D, RuntimeQi4PoolA>();
                Init(ref invalidFifth, ref world);
                Assert.Throws<InvalidOperationException>(() => { invalidFifth.iter_cached_runtime(); });
            }
            finally { world.Dispose(); }
        }
        ...
        public void AllPools_StorageWithoutInlineColumns_EmptyAndAppendSnapshot()
        {
            ...
            try
            {
                var query = new Query<RuntimeQi4PoolA, RuntimeQi4PoolB, RuntimeQi4PoolC, RuntimeQi4PoolD>();
                Init(ref query, ref world);
                var iterator = query.iter_cached_runtime();
                Assert.IsFalse(iterator.MoveNext()); Assert.IsFalse(iterator.MoveNext());
                var poolIterator = query.iter_direct_pool_runtime();
                ...
                query.Update(ref world, IntPtr.Zero);
                var sum = 0;
                foreach (var (a, _, _, _) in query.iter_cached_runtime()) sum += a.Read.Id;
                Assert.AreEqual(3, sum);
                poolIterator = query.iter_direct_pool_runtime();
