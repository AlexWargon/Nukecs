...
{
    // Arity 9; the traversal and tuple representation follow the measured four-component candidate.
    public unsafe struct QueryRuntimeRows9
    {
        public QueryRuntimeColumn C0, C1, C2, C3, C4, C5, C6, C7, C8;
        public int* Entities;
        ...
        public void SetArchetype(ref ArchetypeUnsafe arch)
        {
            ...
            C5.SetArchetype(ref arch);
            C6.SetArchetype(ref arch);
            C7.SetArchetype(ref arch);
            C8.SetArchetype(ref arch);
        ...
        public void LoadPage(int page)
        {
            ...
            if (C5.Pool != null) C5.Base = C5.Page(page);
            if (C6.Pool != null) C6.Base = C6.Page(page);
            if (C7.Pool != null) C7.Base = C7.Page(page);
            if (C8.Pool != null) C8.Base = C8.Page(page);
            PageIndex = page;
        }
    }
    public unsafe interface IQueryRuntimeTuple9
    {
        int Init(ref QueryRuntimeRows9 columns, World.WorldUnsafe* world);
        bool UsesPools();
        ...
        void GatherInline(ref QueryRuntimeRows9 columns, int row);
        void Gather(ref QueryRuntimeRows9 columns, int row);
    }
    public unsafe struct QueryRuntimeRefs<T1, T2, T3, T4, T5, T6, T7, T8, T9> : IQueryRuntimeTuple9
        where T1 : unmanaged where T2 : unmanaged where T3 : unmanaged where T4 : unmanaged where T5 : unmanaged where T6 : unmanaged where T7 : unmanaged where T8 : unmanaged where T9 : unmanaged
    {
        ...
        private static readonly bool Pool5 = QueryRuntimeSlot<T6>.IsPool;
        private static readonly bool Pool6 = QueryRuntimeSlot<T7>.IsPool;
        private static readonly bool Pool7 = QueryRuntimeSlot<T8>.IsPool;
        private static readonly bool Pool8 = QueryRuntimeSlot<T9>.IsPool;
        private static readonly bool HasPools = Pool0 | Pool1 | Pool2 | Pool3 | Pool4 | Pool5 | Pool6 | Pool7 | Pool8;
        private static readonly bool HasSpecial = QueryRuntimeSlot<T1>.IsSpecial | QueryRuntimeSlot<T2>.IsEntity | QueryRuntimeSlot<T3>.IsEntity | QueryRuntimeSlot<T4>.IsEntity | QueryRuntimeSlot<T5>.IsEntity | QueryRuntimeSlot<T6>.IsEntity | QueryRuntimeSlot<T7>.IsEntity | QueryRuntimeSlot<T8>.IsEntity | QueryRuntimeSlot<T9>.IsEntity;
        ...
        public int Init(ref QueryRuntimeRows9 columns, World.WorldUnsafe* world)
        {
            var mask = (columns.C0.Init<T1>(world) ? 1 : 0) | (columns.C1.Init<T2>(world) ? 2 : 0) | (columns.C2.Init<T3>(world) ? 4 : 0) | (columns.C3.Init<T4>(world) ? 8 : 0) | (columns.C4.Init<T5>(world) ? 16 : 0) | (columns.C5.Init<T6>(world) ? 32 : 0) | (columns.C6.Init<T7>(world) ? 64 : 0) | (columns.C7.Init<T8>(world) ? 128 : 0) | (columns.C8.Init<T9>(world) ? 256 : 0);
            if (mask != ((Pool0 ? 1 : 0) | (Pool1 ? 2 : 0) | (Pool2 ? 4 : 0) | (Pool3 ? 8 : 0) | (Pool4 ? 16 : 0) | (Pool5 ? 32 : 0) | (Pool6 ? 64 : 0) | (Pool7 ? 128 : 0) | (Pool8 ? 256 : 0)))
        ...
        {
            p0++;
            if (UsesGather) { p1++; p2++; p3++; p4++; p5++; p6++; p7++; p8++; }
            else
            {
                ...
                if (QueryRuntimeSlot<T6>.IsSpecial) p5 = (T6*)((byte*)p5 - sizeof(T1));
                else if (sizeof(T6) != sizeof(T1)) p5 = (T6*)((byte*)p5 + sizeof(T6) - sizeof(T1));
                if (QueryRuntimeSlot<T7>.IsSpecial) p6 = (T7*)((byte*)p6 - sizeof(T1));
                else if (sizeof(T7) != sizeof(T1)) p6 = (T7*)((byte*)p6 + sizeof(T7) - sizeof(T1));
                if (QueryRuntimeSlot<T8>.IsSpecial) p7 = (T8*)((byte*)p7 - sizeof(T1));
                else if (sizeof(T8) != sizeof(T1)) p7 = (T8*)((byte*)p7 + sizeof(T8) - sizeof(T1));
                if (QueryRuntimeSlot<T9>.IsSpecial) p8 = (T9*)((byte*)p8 - sizeof(T1));
                else if (sizeof(T9) != sizeof(T1)) p8 = (T9*)((byte*)p8 + sizeof(T9) - sizeof(T1));
            }
            return (byte*)p0 < end;
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Set(byte* a0, byte* a1, byte* a2, byte* a3, byte* a4, byte* a5, byte* a6, byte* a7, byte* a8)
        {
            p0 = (T1*)a0;
            if (UsesGather)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void GatherInline(ref QueryRuntimeRows9 columns, int row)
        {
            Set((QueryRuntimeSlot<T1>.IsSpecial ? columns.C0.Base : columns.C0.Base + row * sizeof(T1)), (QueryRuntimeSlot<T2>.IsSpecial ? columns.C1.Base : columns.C1.Base + row * sizeof(T2)), (QueryRuntimeSlot<T3>.IsSpecial ? columns.C2.Base : columns.C2.Base + row * sizeof(T3)), (QueryRuntimeSlot<T4>.IsSpecial ? columns.C3.Base : columns.C3.Base + row * sizeof(T4)), (QueryRuntimeSlot<T5>.IsSpecial ? columns.C4.Base : columns.C4.Base + row * sizeof(T5)), (QueryRuntimeSlot<T6>.IsSpecial ? columns.C5.Base : columns.C5.Base + row * sizeof(T6)), (QueryRuntimeSlot<T7>.IsSpecial ? columns.C6.Base : columns.C6.Base + row * sizeof(T7)), (QueryRuntimeSlot<T8>.IsSpecial ? columns.C7.Base : columns.C7.Base + row * sizeof(T8)), (QueryRuntimeSlot<T9>.IsSpecial ? columns.C8.Base : columns.C8.Base + row * sizeof(T9)));
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Gather(ref QueryRuntimeRows9 columns, int row)
        {
            var slot = 0;
            var entity = 0;
            if (UsesGather)
        ...
        public readonly Ref<T6> _p6 => C5;
        public readonly Ref<T7> _p7 => C6;
        public readonly Ref<T8> _p8 => C7;
        public readonly Ref<T9> _p9 => C8;
        ...
        public readonly Ref<T7> C6 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get => new Ref<T7>(UsesGather ? p6 : (T7*)((byte*)p0 + (long)p6)); }
        public readonly Ref<T8> C7 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get => new Ref<T8>(UsesGather ? p7 : (T8*)((byte*)p0 + (long)p7)); }
        public readonly Ref<T9> C8 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get => new Ref<T9>(UsesGather ? p8 : (T9*)((byte*)p0 + (long)p8)); }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal readonly void DeconstructRefs(out Ref<T1> c0, out Ref<T2> c1, out Ref<T3> c2, out Ref<T4> c3, out Ref<T5> c4, out Ref<T6> c5, out Ref<T7> c6, out Ref<T8> c7, out Ref<T9> c8)
        {
            c0.data = p0;
            if (UsesGather) { c1.data = p1; c2.data = p2; c3.data = p3; c4.data = p4; c5.data = p5; c6.data = p6; c7.data = p7; c8.data = p8; }
            else
            {
                ...
                c5.data = (T6*)((byte*)p0 + (long)p5);
                c6.data = (T7*)((byte*)p0 + (long)p6);
                c7.data = (T8*)((byte*)p0 + (long)p7);
                c8.data = (T9*)((byte*)p0 + (long)p8);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal readonly void DeconstructRefs(out Ref<T1> c0, out Ref<T2> c1, out Ref<T3> c2, out Ref<T4> c3, out Ref<T5> c4, out Ref<T6> c5, out Ref<T7> c6, out Ref<T8> c7)
        {
            c0.data = p0;
            if (UsesGather) { c1.data = p1; c2.data = p2; c3.data = p3; c4.data = p4; c5.data = p5; c6.data = p6; c7.data = p7; }
            else
            {
                c1.data = (T2*)((byte*)p0 + (long)p1);
                c2.data = (T3*)((byte*)p0 + (long)p2);
    ...
    public unsafe ref struct QueryRuntimeIter9<TTuple> where TTuple : unmanaged, IQueryRuntimeTuple9
    {
        private TTuple current;
        private QueryRuntimeRows9 columns;
        ...
        public QueryRuntimeIter9(QueryUnsafe* query) : this(query, 0, int.MaxValue, true) { }
        public QueryRuntimeIter9(QueryUnsafe* query, in Range range) : this(query, range.start, range.end, false) { }
        private QueryRuntimeIter9(QueryUnsafe* query, int start, int stop, bool allowStorage)
        {
        ...
        public readonly QueryRuntimeIter9<TTuple> GetEnumerator() => this;
