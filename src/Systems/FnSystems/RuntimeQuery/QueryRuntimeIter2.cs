...
{
    // Arity 2; the traversal and tuple representation follow the measured four-component candidate.
    public unsafe struct QueryRuntimeRows2
    {
        public QueryRuntimeColumn C0, C1;
        public int* Entities;
    ...
    public unsafe interface IQueryRuntimeTuple2
    {
        int Init(ref QueryRuntimeRows2 columns, World.WorldUnsafe* world);
        bool UsesPools();
        ...
        void GatherInline(ref QueryRuntimeRows2 columns, int row);
        void Gather(ref QueryRuntimeRows2 columns, int row);
    }
    public unsafe struct QueryRuntimeRefs<T1, T2> : IQueryRuntimeTuple2
        where T1 : unmanaged where T2 : unmanaged
    {
        ...
        private static readonly bool HasPools = Pool0 | Pool1;
        private static readonly bool HasSpecial = QueryRuntimeSlot<T1>.IsSpecial | QueryRuntimeSlot<T2>.IsEntity;
        ...
        public int Init(ref QueryRuntimeRows2 columns, World.WorldUnsafe* world)
        {
            var mask = (columns.C0.Init<T1>(world) ? 1 : 0) | (columns.C1.Init<T2>(world) ? 2 : 0);
            if (mask != ((Pool0 ? 1 : 0) | (Pool1 ? 2 : 0)))
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Set(byte* a0, byte* a1)
        {
            p0 = (T1*)a0;
            if (UsesGather)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void GatherInline(ref QueryRuntimeRows2 columns, int row)
        {
            Set((QueryRuntimeSlot<T1>.IsSpecial ? columns.C0.Base : columns.C0.Base + row * sizeof(T1)), (QueryRuntimeSlot<T2>.IsSpecial ? columns.C1.Base : columns.C1.Base + row * sizeof(T2)));
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Gather(ref QueryRuntimeRows2 columns, int row)
        {
            var slot = 0;
            var entity = 0;
            if (UsesGather)
            ...
            Set((QueryRuntimeSlot<T1>.IsSpecial ? columns.C0.SpecialAddress<T1>(entity) : columns.C0.Base + (Pool0 ? slot : row) * sizeof(T1)), (QueryRuntimeSlot<T2>.IsSpecial ? columns.C1.SpecialAddress<T2>(entity) : columns.C1.Base + (Pool1 ? slot : row) * sizeof(T2)));
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal readonly void DeconstructRefs(out Ref<T1> c0, out Ref<T2> c1)
        {
            c0.data = p0;
    ...
    public unsafe ref struct QueryRuntimeIter2<TTuple> where TTuple : unmanaged, IQueryRuntimeTuple2
    {
        private TTuple current;
        private QueryRuntimeRows2 columns;
        ...
        public QueryRuntimeIter2(QueryUnsafe* query) : this(query, 0, int.MaxValue, true) { }
        public QueryRuntimeIter2(QueryUnsafe* query, in Range range) : this(query, range.start, range.end, false) { }
        private QueryRuntimeIter2(QueryUnsafe* query, int start, int stop, bool allowStorage)
        {
        ...
        public readonly QueryRuntimeIter2<TTuple> GetEnumerator() => this;
