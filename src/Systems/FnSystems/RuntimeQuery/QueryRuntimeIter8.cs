...
{
    // Arity 8; the traversal and tuple representation follow the measured four-component candidate.
    public unsafe struct QueryRuntimeRows8
    {
        public QueryRuntimeColumn C0, C1, C2, C3, C4, C5, C6, C7;
        public int* Entities;
    ...
    public unsafe interface IQueryRuntimeTuple8
    {
        int Init(ref QueryRuntimeRows8 columns, World.WorldUnsafe* world);
        bool UsesPools();
        ...
        void GatherInline(ref QueryRuntimeRows8 columns, int row);
        void Gather(ref QueryRuntimeRows8 columns, int row);
    }
    public unsafe struct QueryRuntimeRefs<T1, T2, T3, T4, T5, T6, T7, T8> : IQueryRuntimeTuple8
        where T1 : unmanaged where T2 : unmanaged where T3 : unmanaged where T4 : unmanaged where T5 : unmanaged where T6 : unmanaged where T7 : unmanaged where T8 : unmanaged
    {
        ...
        private static readonly bool HasPools = Pool0 | Pool1 | Pool2 | Pool3 | Pool4 | Pool5 | Pool6 | Pool7;
        private static readonly bool HasSpecial = QueryRuntimeSlot<T1>.IsSpecial | QueryRuntimeSlot<T2>.IsEntity | QueryRuntimeSlot<T3>.IsEntity | QueryRuntimeSlot<T4>.IsEntity | QueryRuntimeSlot<T5>.IsEntity | QueryRuntimeSlot<T6>.IsEntity | QueryRuntimeSlot<T7>.IsEntity | QueryRuntimeSlot<T8>.IsEntity;
        ...
        public int Init(ref QueryRuntimeRows8 columns, World.WorldUnsafe* world)
        {
            var mask = (columns.C0.Init<T1>(world) ? 1 : 0) | (columns.C1.Init<T2>(world) ? 2 : 0) | (columns.C2.Init<T3>(world) ? 4 : 0) | (columns.C3.Init<T4>(world) ? 8 : 0) | (columns.C4.Init<T5>(world) ? 16 : 0) | (columns.C5.Init<T6>(world) ? 32 : 0) | (columns.C6.Init<T7>(world) ? 64 : 0) | (columns.C7.Init<T8>(world) ? 128 : 0);
            if (mask != ((Pool0 ? 1 : 0) | (Pool1 ? 2 : 0) | (Pool2 ? 4 : 0) | (Pool3 ? 8 : 0) | (Pool4 ? 16 : 0) | (Pool5 ? 32 : 0) | (Pool6 ? 64 : 0) | (Pool7 ? 128 : 0)))
        ...
        {
            p0++;
            if (UsesGather) { p1++; p2++; p3++; p4++; p5++; p6++; p7++; }
            else
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Set(byte* a0, byte* a1, byte* a2, byte* a3, byte* a4, byte* a5, byte* a6, byte* a7)
        {
            p0 = (T1*)a0;
            if (UsesGather)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal readonly void DeconstructRefs(out Ref<T1> c0, out Ref<T2> c1, out Ref<T3> c2, out Ref<T4> c3, out Ref<T5> c4, out Ref<T6> c5, out Ref<T7> c6)
        {
            c0.data = p0;
            if (UsesGather) { c1.data = p1; c2.data = p2; c3.data = p3; c4.data = p4; c5.data = p5; c6.data = p6; }
            else
            {
                c1.data = (T2*)((byte*)p0 + (long)p1);
                c2.data = (T3*)((byte*)p0 + (long)p2);
    ...
    public unsafe ref struct QueryRuntimeIter8<TTuple> where TTuple : unmanaged, IQueryRuntimeTuple8
    {
        private TTuple current;
        private QueryRuntimeRows8 columns;
        ...
        public QueryRuntimeIter8(QueryUnsafe* query) : this(query, 0, int.MaxValue, true) { }
        public QueryRuntimeIter8(QueryUnsafe* query, in Range range) : this(query, range.start, range.end, false) { }
        private QueryRuntimeIter8(QueryUnsafe* query, int start, int stop, bool allowStorage)
        {
        ...
        public readonly QueryRuntimeIter8<TTuple> GetEnumerator() => this;
