...
{
    // Arity 7; the traversal and tuple representation follow the measured four-component candidate.
    public unsafe struct QueryRuntimeRows7
    {
        public QueryRuntimeColumn C0, C1, C2, C3, C4, C5, C6;
        public int* Entities;
    ...
    public unsafe interface IQueryRuntimeTuple7
    {
        int Init(ref QueryRuntimeRows7 columns, World.WorldUnsafe* world);
        bool UsesPools();
        ...
        void GatherInline(ref QueryRuntimeRows7 columns, int row);
        void Gather(ref QueryRuntimeRows7 columns, int row);
    }
    public unsafe struct QueryRuntimeRefs<T1, T2, T3, T4, T5, T6, T7> : IQueryRuntimeTuple7
        where T1 : unmanaged where T2 : unmanaged where T3 : unmanaged where T4 : unmanaged where T5 : unmanaged where T6 : unmanaged where T7 : unmanaged
    {
        ...
        private static readonly bool HasPools = Pool0 | Pool1 | Pool2 | Pool3 | Pool4 | Pool5 | Pool6;
        private static readonly bool HasSpecial = QueryRuntimeSlot<T1>.IsSpecial | QueryRuntimeSlot<T2>.IsEntity | QueryRuntimeSlot<T3>.IsEntity | QueryRuntimeSlot<T4>.IsEntity | QueryRuntimeSlot<T5>.IsEntity | QueryRuntimeSlot<T6>.IsEntity | QueryRuntimeSlot<T7>.IsEntity;
        ...
        public int Init(ref QueryRuntimeRows7 columns, World.WorldUnsafe* world)
        {
            var mask = (columns.C0.Init<T1>(world) ? 1 : 0) | (columns.C1.Init<T2>(world) ? 2 : 0) | (columns.C2.Init<T3>(world) ? 4 : 0) | (columns.C3.Init<T4>(world) ? 8 : 0) | (columns.C4.Init<T5>(world) ? 16 : 0) | (columns.C5.Init<T6>(world) ? 32 : 0) | (columns.C6.Init<T7>(world) ? 64 : 0);
            if (mask != ((Pool0 ? 1 : 0) | (Pool1 ? 2 : 0) | (Pool2 ? 4 : 0) | (Pool3 ? 8 : 0) | (Pool4 ? 16 : 0) | (Pool5 ? 32 : 0) | (Pool6 ? 64 : 0)))
        ...
        {
            p0++;
            if (UsesGather) { p1++; p2++; p3++; p4++; p5++; p6++; }
            else
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Set(byte* a0, byte* a1, byte* a2, byte* a3, byte* a4, byte* a5, byte* a6)
        {
            p0 = (T1*)a0;
            if (UsesGather)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void GatherInline(ref QueryRuntimeRows7 columns, int row)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Gather(ref QueryRuntimeRows7 columns, int row)
        {
            var slot = 0;
            var entity = 0;
            if (UsesGather)
    ...
    public unsafe ref struct QueryRuntimeIter7<TTuple> where TTuple : unmanaged, IQueryRuntimeTuple7
    {
        private TTuple current;
        private QueryRuntimeRows7 columns;
        ...
        public QueryRuntimeIter7(QueryUnsafe* query) : this(query, 0, int.MaxValue, true) { }
        public QueryRuntimeIter7(QueryUnsafe* query, in Range range) : this(query, range.start, range.end, false) { }
        private QueryRuntimeIter7(QueryUnsafe* query, int start, int stop, bool allowStorage)
        {
        ...
        public readonly QueryRuntimeIter7<TTuple> GetEnumerator() => this;
