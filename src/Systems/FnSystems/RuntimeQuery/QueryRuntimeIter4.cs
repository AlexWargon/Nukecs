...
{
    // Arity 4; the traversal and tuple representation follow the measured four-component candidate.
    public unsafe struct QueryRuntimeRows4
    {
        public QueryRuntimeColumn C0, C1, C2, C3;
        public int* Entities;
    ...
    public unsafe interface IQueryRuntimeTuple4
    {
        int Init(ref QueryRuntimeRows4 columns, World.WorldUnsafe* world);
        bool UsesPools();
        ...
        void GatherInline(ref QueryRuntimeRows4 columns, int row);
        void Gather(ref QueryRuntimeRows4 columns, int row);
    }
    public unsafe struct QueryRuntimeRefs<T1, T2, T3, T4> : IQueryRuntimeTuple4
        where T1 : unmanaged where T2 : unmanaged where T3 : unmanaged where T4 : unmanaged
    {
        ...
        {
            var mask = (columns.C0.Init<T1>(world) ? 1 : 0) | (columns.C1.Init<T2>(world) ? 2 : 0) | (columns.C2.Init<T3>(world) ? 4 : 0) | (columns.C3.Init<T4>(world) ? 8 : 0);
            if (mask != ((Pool0 ? 1 : 0) | (Pool1 ? 2 : 0) | (Pool2 ? 4 : 0) | (Pool3 ? 8 : 0)))
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Set(byte* a0, byte* a1, byte* a2, byte* a3)
        {
            p0 = (T1*)a0;
            if (UsesGather)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void GatherInline(ref QueryRuntimeRows4 columns, int row)
        {
            Set((QueryRuntimeSlot<T1>.IsSpecial ? columns.C0.Base : columns.C0.Base + row * sizeof(T1)), (QueryRuntimeSlot<T2>.IsSpecial ? columns.C1.Base : columns.C1.Base + row * sizeof(T2)), (QueryRuntimeSlot<T3>.IsSpecial ? columns.C2.Base : columns.C2.Base + row * sizeof(T3)), (QueryRuntimeSlot<T4>.IsSpecial ? columns.C3.Base : columns.C3.Base + row * sizeof(T4)));
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Gather(ref QueryRuntimeRows4 columns, int row)
        {
            var slot = 0;
            var entity = 0;
            if (UsesGather)
    ...
    public unsafe ref struct QueryRuntimeIter4<TTuple> where TTuple : unmanaged, IQueryRuntimeTuple4
    {
        private TTuple current;
        private QueryRuntimeRows4 columns;
        ...
        public QueryRuntimeIter4(QueryUnsafe* query) : this(query, 0, int.MaxValue, true) { }
        public QueryRuntimeIter4(QueryUnsafe* query, in Range range) : this(query, range.start, range.end, false) { }
        private QueryRuntimeIter4(QueryUnsafe* query, int start, int stop, bool allowStorage)
        {
        ...
        public readonly QueryRuntimeIter4<TTuple> GetEnumerator() => this;
